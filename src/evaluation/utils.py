...
def evaluate_move(fen_position: str, move: str):
    with open("data/evaluated_positions.json", "r") as f:
        d = json.load(f)
    engine_list_moves = d[fen_position]
    index = engine_list_moves.index(move)
    # max 10 for bets move, min -10 for worst move, 0 avg move
    evaluation = (1-index/len(engine_list_moves))*20-10
    return evaluation

def generate_dict_of_evaluated_positions():
    from stockfish import Stockfish
    d = {}
    engine = Stockfish("/opt/homebrew/bin/stockfish")
    for fen_position in FEN_POSITIONS:
        legal_moves_length = len(Chess(fen_position).legal_moves())
        engine.set_fen_position(fen_position)
        top_moves = engine.get_top_moves(legal_moves_length)
        top_moves_cleaned = [move_dict["Move"] for move_dict in top_moves]
        
        d[fen_position] = top_moves_cleaned
        print("Generated evaluated positions for ", fen_position)

    with open("data/evaluated_positions.json", "w") as f:
        json.dump(d, f)

if __name__=="__main__":
    generate_dict_of_evaluated_positions()
    #print(evaluate_move("1k1r4/pp1b1R2/3q2pp/4p3/2B5/4Q3/PPP2B2/2K5 b - - 0 0","d6d1"))